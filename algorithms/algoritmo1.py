¿puedo saber por qué mis ventanas de tiempo ponen 8:45 - 4:15? en lugar de 8:15 - 17:15 considerando el horario cambiado???
...
import googlemaps
from googlemaps.convert import decode\_polyline
from ortools.constraint\_solver import pywrapcp, routing\_enums\_pb2
...
## Usa el JSON de servicio: 'lavanderia\_key.json'

\#if not firebase\_admin.\_apps:

# cred = credentials.Certificate("lavanderia\_key.json")
...
GOOGLE\_MAPS\_API\_KEY = st.secrets.get("google\_maps", {}).get("api\_key") or os.getenv("GOOGLE\_MAPS\_API\_KEY")
gmaps = googlemaps.Client(key=GOOGLE\_MAPS\_API\_KEY)

# -------------------- CONSTANTES VRP --------------------

SERVICE\_TIME    = 8 \* 60        # 8 minutos de servicio en cada parada (excepto depósito)
MAX\_ELEMENTS    = 100            # límite de celdas por petición Distance Matrix API
SHIFT\_START\_SEC =  8*3600 + 30*60   # 08:30 en segundos
SHIFT\_END\_SEC   = 17\*3600           # 17:00 en segundos
MARGEN = 15 \* 60  # 15 minutos en segundos

# ===================== FUNCIONES AUXILIARES =====================

def \_hora\_a\_segundos(hhmm):
...
def \_haversine\_dist\_dur(coords, vel\_kmh=40.0):
"""
Calcula matrices de distancias (en metros) y duraciones (en segundos)
basadas en fórmula de Haversine asumiendo velocidad vel\_kmh para la duración.
...
v\_ms = vel\_kmh \* 1000 / 3600  # convertir km/h a m/s
for i in range(n):
for j in range(n):
if i == j:
continue
...
@st.cache\_data(ttl=3600, show\_spinner=False)
def \_distancia\_duracion\_matrix(coords):
"""
...
"""
if not GOOGLE\_MAPS\_API\_KEY:
return \_haversine\_dist\_dur(coords)
n = len(coords)
dist = \[\[0]\*n for \_ in range(n)]
dur  = \[\[0]\*n for \_ in range(n)]
\# Dividimos en lotes para no exceder MAX\_ELEMENTS celdas
batch = max(1, min(n, MAX\_ELEMENTS // n))
for i0 in range(0, n, batch):
resp = gmaps.distance\_matrix(
origins=coords\[i0\:i0+batch],
destinations=coords,
mode="driving",
units="metric",
departure\_time=datetime.now(),
traffic\_model="best\_guess"
)
for i, row in enumerate(resp\["rows"]):
for j, el in enumerate(row\["elements"]):
dist\[i0 + i]\[j] = el.get("distance", {}).get("value", 1)
dur\[i0 + i]\[j]  = el.get("duration\_in\_traffic", {}).get(
"value",
el.get("duration", {}).get("value", 1)
)
return dist, dur

def \_crear\_data\_model(df, vehiculos=1, capacidad\_veh=None):
coords = list(zip(df\["lat"], df\["lon"]))
dist\_m, dur\_s = \_distancia\_duracion\_matrix(coords)
...
def optimizar\_ruta\_algoritmo22(data, tiempo\_max\_seg=60, reintento=False):
"""
VRPTW con OR-Tools (Routing Solver):
\- service\_time se suma en el callback (en el nodo origen)
\- no fijamos ventana del depósito en el bucle; se fija aparte a SHIFT\_START\_SEC
\- reintento ampliando ventanas cortas si falla
"""
manager = pywrapcp.RoutingIndexManager(
len(data\["distance\_matrix"]),
data\["num\_vehicles"],
data\["depot"]
)
...
def agregar\_ventana\_margen(df, margen\_segundos=15*60):
def expandir\_fila(row):
ini = \_hora\_a\_segundos(row\["time\_start"])
fin = \_hora\_a\_segundos(row\["time\_end"])
if ini is None or fin is None:
return "No especificado"
ini = max(0, ini - margen\_segundos)
fin = min(24*3600, fin + margen\_segundos)
h\_ini = f"{ini//3600:02}:{(ini%3600)//60:02}"
h\_fin = f"{fin//3600:02}:{(fin%3600)//60:02}"
return f"{h\_ini} - {h\_fin} h"
...
def \_haversine\_meters(lat1, lon1, lat2, lon2):
...
def agrupar\_puntos\_aglomerativo(df, eps\_metros=5):
"""
Agrupa pedidos cercanos mediante AgglomerativeClustering.
eps\_metros: umbral de distancia máxima en metros para que queden en el mismo cluster.
Retorna (df\_clusters, df\_etiquetado), donde:
\- df\_etiquetado = df original con columna 'cluster' indicando etiqueta de cluster.
\- df\_clusters  = DataFrame de centroides con columnas \['id','operacion','nombre\_cliente',
'dirección','lat','lon','time\_start','time\_end','demand'].
"""
...
def cargar\_pedidos(fecha, tipo):
"""

...
# Clarke–Wright + Tabu Search para VRP-TW
...
def _route_distance(route: List[int], data: Dict[str, Any]) -> float:
    """Suma de distancias (m) a lo largo de una ruta."""
...
def _check_feasible_and_time(route: List[int], data: Dict[str, Any]) -> Tuple[bool, List[int]]:
    """
    Verifica si una ruta es factible bajo ventanas de tiempo.
    Devuelve (es_factible, tiempos_llegada_sec).
    """
...
def optimizar_ruta_cw_tabu(data: Dict[str, Any], tiempo_max_seg: int = 60) -> Dict[str, Any]:
    """
    1. Aplica Clarke–Wright Savings para formar mini-rutas.
    2. Refina cada una con Tabu Search (intercambio de nodos).
    3. Une todas en una sola ruta con validación de ventanas de tiempo.
    """
    depot = data["depot"]
    n = len(data["distance_matrix"])
    nodes = [i for i in range(n) if i != depot]

    # Etapa 1: Clarke–Wright Savings
    D = data["distance_matrix"]
    savings = []
    for i in nodes:
        for j in nodes:
    ...
    # Etapa 3: Unir todo en una sola ruta
    clientes = []
    ...
    if not feas or len(ruta_final) != len(llegada_final):
        llegada_final = [SHIFT_START_SEC + i * 90 for i in range(len(ruta_final))]

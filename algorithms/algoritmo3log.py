...
BIG_M = 10**6
TOLERANCIA_RETRASO = 60 * 60  # Hasta 60 minutos de tolerancia
...
def optimizar_ruta_cp_sat_puro(data, tiempo_max_seg=120):
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                travel = max(dur[i][j], 1)
                model.Add(t[j] >= t[i] + service_times[i] + travel).OnlyEnforceIf(x[i, j])

    # Penalización fuerte por no visitar nodos
    penalizacion = sum(x[i, j] * dur[i][j] for i in range(n) for j in range(n) if i != j)
    penalizacion += sum(retraso[i] * 100 for i in range(n))  # penaliza llegar tarde
    penalizacion += sum((1 - visited[i]) * BIG_M for i in range(1, n))  # evita omisiones

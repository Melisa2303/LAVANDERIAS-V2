...
def optimizar_ruta_cp_sat_puro(data, tiempo_max_seg=60):
    ...
    t = [model.NewIntVar(0, MAX_TIEMPO_TOTAL, f"t_{i}") for i in range(n)]
    retraso = [model.NewIntVar(0, TOLERANCIA_RETRASO, f"ret_{i}") for i in range(n)]

    # Flujo: entrar y salir exactamente una vez por nodo
    for j in range(1, n):
        model.Add(sum(x[i, j] for i in range(n) if i != j) == 1)
    for i in range(1, n):
        model.Add(sum(x[i, j] for j in range(n) if i != j) == 1)

    # Depósito (solo uno entra y uno sale)
    ...
    for i in range(n):
        ini, fin = ventanas[i]
        model.Add(t[i] >= ini)
        model.Add(t[i] <= fin + retraso[i])  # se permite cierto retraso penalizado

    # Secuencia de tiempo entre nodos
    for i in range(n):
        for j in range(n):
            if i != j:
                model.Add(t[j] >= t[i] + service_times[i] + dur[i][j]).OnlyEnforceIf(x[i, j])

    # Función objetivo: minimizar duración + retrasos
    model.Minimize(
        sum(dur[i][j] * x[i, j] for i in range(n) for j in range(n) if i != j) +
        sum(retraso[i] * 20 for i in range(n))  # penalización suave por llegar tarde
    )
